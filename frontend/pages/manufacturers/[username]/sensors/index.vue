...
const { data: sensors, error, refresh } = await useFetch(`${api}/sensors/source/Product`)
...
const addSensor = () => navigateTo(`/manufacturers/${username}/sensors/create`)
const editItem = (item) => {
    navigateTo(`/manufacturers/${username}/sensors/${item.id}/edit`)
}
